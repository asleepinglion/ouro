      ...
      if( path.length > 1 ) {
        ...
        if( !req.action ) {
          if( req.actionType === 'REST' ) {
            return reject(new SuperJS.Error('action_not_found', 404, 'REST Controller action ' + req.method + ' not implemented.'));
          } else {
            return reject(new SuperJS.Error('action_not_found', 404, 'Controller RPC action ' + req.method + ' not found.'));
          }
        }
      ...
      if( typeof self.config.server.mock === 'object' &&
        ...
        if(  self.config.server.mock.specific === false ||
          ...
          if (self.config.server.mock.required !== true && !req.param('_mock') ) {
            ...
            reject(new SuperJS.Error('mock_not_found', 401, 'The mock of the ' + mockFile + ' response for the ' + req.controller + '.' + req.action + ' action was not enabled.'));
          }
        }
