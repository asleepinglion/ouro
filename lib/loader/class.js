...
module.exports = Class.extend({
  ...
  _load: function(className, options) {
    ...
    options.setup.modulePrefix = options.setup.modulePrefix = 'ouro-';
    ...
    if( options.setup.local ) {
      ...
      if( typeof global.ouro[options.setup.namespace] !== 'object' ) {
        global.ouro[options.setup.namespace] = {};
      }

      //if the class already exists, only replace it if the replace option is present
      if( typeof global.ouro[options.setup.namespace][className] === 'object' && !options.setup.replace ) {

        //add local alias to the instance if requested
        if( typeof options.setup.alias === 'string' ) {
          this[options.setup.alias] = global.ouro[options.setup.namespace][className];
        }
    ...
    //setup current working directory path
    var cwdClassPath = process.cwd()  + '/' + options.setup.classDir + '/' + className + '/' + options.setup.classFile;
    ...
    if( fs.existsSync( cwdClassPath ) ) {

      //use the cwd class if it exists there
      classPath = cwdClassPath;

    } else if( fs.existsSync( classPath ) ) {
    ...
    //otherwise, mount into the global namespace
      ...
      global.ouro[options.setup.namespace][className] = instance;
    }
  ...
  _class: function(className, namespace) {
    ...
    } else if( global.ouro && global.ouro[namespace] && global.ouro[namespace][className] ) {
      return global.ouro[namespace][className];
    } else {
      return undefined;
    }
  }
