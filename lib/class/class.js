...
module.exports = Class.extend(EventEmitter, Meta, Loader, {

  init: function() {

    //load essential modules that all classes should be able to use
    this._load('console', { setup: { modulePath: this._modulePath(__dirname), shortcut: 'log'}, boot: { objectDepth: 5 } });
    this._load('transform', { setup: { modulePath: this._modulePath(__dirname), shortcut: 'transform' }});
    this._load('validate', { setup: { modulePath: this._modulePath(__dirname), shortcut: 'validate' }});
