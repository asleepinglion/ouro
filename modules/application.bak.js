...
module.exports = SuperJS.Class.extend({
  ...
  init: function() {
    ...
    this.configPath = (typeof this.configPath === 'string') ? this.configPath : '/configuration';
  ...
  configure: function() {
    ...
    //attempt to load package configuration
    this.loadConfig('package.json', this.appPath, true);

    //attempt to load server configuration
    this.loadConfig('server', this.appPath + this.configPath, true);
    this.loadConfig('server', this.appPath + this.configPath + '/' + this.environment, false);

    //attempt to load security configuration
    this.loadConfig('security', this.appPath + this.configPath, true);
    this.loadConfig('security', this.appPath + this.configPath + '/' + this.environment, false);

    //attempt to load security configuration
    this.loadConfig('data', this.appPath + this.configPath, true);
    this.loadConfig('data', this.appPath + this.configPath + '/' + this.environment, false);

    //get list of service configuration files
    var services = fs.readdirSync(this.appPath + this.configPath + '/services');

    //load each service configuration
    services.map(function(serviceName) {
      self.loadConfig(serviceName, self.appPath + self.configPath + '/services', true);
    });
  ...
  loadConfig: function(name, path, required) {

    var filename = ( name.split('.').length > 1 ) ? name : name + '.js';

    //attempt to load server configuration
    if( fs.existsSync(path + '/' + filename) ) {

      this.log.debug('loading configuration:', path + '/' + name);

      if( typeof this.config[name] === 'object' ) {
        this.config[name] = merge(this.config[name], require(path + '/' + name));
      } else {
        this.config[name] = require(path + '/' + name);
      }

    } else if( required ) {
      throw new SuperJS.Error('missing_configuration', 'The ' + name + ' configuration is required (' + path + '/' + filename + ').');
    }
  ...
  processBlueprint: function(controllerName, blueprints)  {
    ...
    //localize the blueprint for easier access
      ...
      for( var param in blueprint.actions[action].params ) {
        ...
        if( blueprint.actions[action].params[param].model ) {
          ...
          if( blueprint.actions[action].params[param].model === true ) {
            ...
            if( this.models[controllerName] && this.models[controllerName].attributes) {
  ...
  executeRequest: function(req, res) {
    ...
    //localize the method name
    var methodName = self.externalMethods[req.controller][req.action]._method;

    //emit beforeAction events for secondary operations
    self.controllers[req.controller].emit('beforeAction', req);
    self.controllers[req.controller].emit('before' + methodName, req);
    ...
    return self.mockResponse(req, res)
      ...
      .then(function() {
        ...
        if( !res.responded ) {
          ...
          return self.controllers[req.controller].beforeAction(req)
            ...
            .then(function() {
              return self.controllers[req.controller][methodName](req);
            })
      ...
      .then(function() {
        ...
        self.controllers[req.controller].emit('after' + methodName, req);
  ...
  //override this to manipulate the request or resposne
